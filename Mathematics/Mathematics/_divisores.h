...
#include "_fatoresPrimos.h"
#include "_prime.h"

namespace math {
	// Qndo a divis�o de um n. por outro � exata este n. � divis�vel pelo outro.
	// -O n. 1 � divis�vel por qualquer n�mero.
	// -O zero n�o � divisor de n. nenhum.
	// -Todo n. � divisor de si mesmo.
	// Vers�o FAST:
	template <typename T>
	...
	// For�a (B)ruta: M�ximo dividendo 4294967295 (unsigned int / long / long int - 4-bytes)
	...
	static void _divisoresUsage() {
		...
		utils::PrintColour_("<<__DIVISORESs__>>", FBI);
		utils::dicShow("mathematics", "Divisores");

		vetorULLng v = _divisoresD(static_cast<unsigned long long>(999));
		//_testShow(v, 9999999999);
		system("pause");

		//Static (D)idatic version:
		//vetorULLng v = _divisoresD(MAX); // Devolve uma c�pia de objs vetores criado estaticamente no STACK
		//_testShow(v, MAX);
/*
		// Vers�o FAST (static-memory):
		vetorULLng v0 = _divisores(static_cast<unsigned long long>(54));
		_testShow(v0, 54);

		//Static For�a (B)ruta version:
		// ERROR unsigned long long will run FOREVER!:
		vetorULLng v2 = _divisoresB(static_cast<unsigned long long>(54)); // Devolve uma c�pia de objs vetores criado estaticamente no STACK
		_testShow(v2, 54);
		...
		vetorULLng v4 = _divisoresD(static_cast<unsigned long long>(54)); // Devolve uma c�pia de objs vetores criado estaticamente no STACK
		_testShow(v4, 54);
*/
		MIN = 999999;
		MAX = 10000000;
	...
	static int _dividersQuantity(unsigned long long _n) {
		vetorULLng vl = _fatoresPrimos2(_n);
		//utils::_showVetor(vl);
